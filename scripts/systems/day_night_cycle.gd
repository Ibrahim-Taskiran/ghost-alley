...
@export var time_scale: float = 1.0 # 20 minutes day / 4 minutes night (1 game hour = 1 real minute at 1.0)
@export var day_start_hour: float = 4.0
@export var night_start_hour: float = 24.0
@export var is_cycle_active: bool = true # Set to false to freeze daylight
@export var frozen_hour: float = 12.0 # Midday (noon) by default

@export_category("Node References")
@export var sun_light: DirectionalLight3D
@export var world_env: WorldEnvironment

var current_hour: float = 4.0
var current_day: int = 1
var is_night: bool = false

# Visual lerp targets
var day_sky_top = Color(0.35, 0.38, 0.48)
var night_sky_top = Color(0.01, 0.02, 0.05) # Much darker night sky
var day_sky_horizon = Color(0.55, 0.52, 0.48)
var night_sky_horizon = Color(0.02, 0.03, 0.06) # Much darker horizon

var day_ambient_color = Color(0.5, 0.5, 0.55)
var night_ambient_color = Color(0.04, 0.05, 0.08) # Dark night ambient

func _ready() -> void:
	add_to_group("DayNightCycle")
	if not is_cycle_active:
		current_hour = frozen_hour
...
func _process(delta: float) -> void:
	if not is_cycle_active:
		current_hour = frozen_hour
		is_night = false # Always day
		_update_time_visuals(delta)
		
		# Still emit hourly signal to keep HUD time consistent
...
func _update_time_visuals(_delta: float) -> void:
	...
	if not is_night:
		...
		light_factor = sin(factor * PI)
		sun_light.light_energy = lerp(0.08, 0.35, light_factor) # Brighter moonlight
		sun_light.light_color = Color(0.4, 0.5, 0.7) # Silvery blue light
	...
	if world_env and world_env.environment:
		...
		if not is_night:
			...
			env.ambient_light_energy = lerp(0.05, 0.15, light_factor) # Lowered ambient for actual dark feeling between 00:00 and 04:00
